...
void apply_model_generate(PhiModel *model, PhiModelRunState *state, PhiModelInput *input, float *embedded_tokens, unsigned int batch_size, unsigned int generation_offset) {
    PhiDecoderLayer *decoder_layer;
    PhiAttention *attn;
    float *decoder_input = embedded_tokens;
    float *pre_ln_result, *query_states, *key_states, *value_states, *query_rot, *key_rot;
    for (unsigned int layer_idx = 0; layer_idx < model->config->num_hidden_layers; ++layer_idx) {
        decoder_layer = model->decoder_layers[layer_idx];
        attn = decoder_layer->attention_layer;
        layernorm_op(decoder_layer->preln->gamma, decoder_layer->preln->beta, decoder_layer->preln->epsilon, decoder_input, pre_ln_result, decoder_layer->preln->hidden_size, batch_size);
        linear_op_omp_simd(attn->q_proj->weight, attn->q_proj->bias, pre_ln_result, query_states, attn->q_proj->fan_in, attn->q_proj->fan_out, batch_size);
        linear_op_omp_simd(attn->k_proj->weight, attn->k_proj->bias, pre_ln_result, query_states, attn->k_proj->fan_in, attn->k_proj->fan_out, batch_size);
        linear_op_omp_simd(attn->v_proj->weight, attn->v_proj->bias, pre_ln_result, query_states, attn->v_proj->fan_in, attn->v_proj->fan_out, batch_size);
        rotary_op_gen(attn->remb->sin, attn->remb->cos, query_states, query_rot, attn->remb->rotary_dim, model->config->head_dim, model->config->num_attention_heads, batch_size, input->seq_lens, generation_offset);
        rotary_op_gen(attn->remb->sin, attn->remb->cos, key_states, key_rot, attn->remb->rotary_dim, model->config->head_dim, model->config->num_attention_heads, batch_size, input->seq_lens, generation_offset);
